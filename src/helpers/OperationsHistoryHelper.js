...
const formatOperation = async (operationData) => {
	...
	switch (operationData.op[0]) {
		case operations.transfer: {
			...
			result.time = (await FetchChain('getBlock', operationData.block_num)).timestamp;
			break;
		}
		...
		case operations.limit_order_cancel:
			...
			result.subject = (await FetchChain('fetchObject', operation.order)).toJS().name;
			result.status = 'success';
			break;
